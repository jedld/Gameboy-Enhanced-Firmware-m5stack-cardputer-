...
static void show_options_menu() {
  ...
  set_font_size(200);
  M5Cardputer.Display.println("J/S=Down  K/W=Up");
  M5Cardputer.Display.println("L/ENTER=Select  H=Prev (cache)");
    }
  ...
  Entry e{String(entry.name()), entry.isDirectory(), false, SIZE_MAX, nullptr};
    ...
    while(!selection_made) {
      ...
      if(redraw) {
        ...
        const uint16_t bg_selected = M5Cardputer.Display.color565(40, 70, 120);
        ...
        for(int row = 0; row < visible_rows; ++row) {
          ...
          uint16_t row_bg = is_selected ? bg_selected : bg_default;
          uint16_t row_fg = is_selected ? fg_selected : fg_default;

          M5Cardputer.Display.fillRect(0, y, dispW, lineHeight, row_bg);
          M5Cardputer.Display.setTextColor(row_fg, row_bg);
        ...
        M5Cardputer.Display.setTextColor(M5Cardputer.Display.color565(255, 255, 255), bg_default);
      }
    }
