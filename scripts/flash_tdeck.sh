...
usage() {
    cat <<'EOF'
Usage: scripts/flash_tdeck.sh [OPTIONS]

Build the LilyGO T-Deck firmware with PlatformIO and flash it to an attached device.

Options:
  -p, --port <device>   Serial device to flash (e.g. /dev/ttyACM0). Auto-detected when omitted.
  -e, --env <name>      PlatformIO environment to build (default: lilygo_tdeck).
